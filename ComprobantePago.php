<?php
    ...
    if (isset($_SESSION['NumEmpleado5'])) {
        ...
        $sql = "SELECT Nombres, ApellidoPat, ApellidoMat, Cantidad, IdInversionista, Division FROM empleado inner join inversionista on empleado.NumEmpleado = inversionista. NumEmpleado6 inner join division on empleado.IdDivision1 = division.IdDivision WHERE NumEmpleado = '$NumEmpleado' ";
...
$pdf->Ln(10);
$pdf->MultiCell(0, 7, utf8_decode('El que suscribe servidor público '.$row[0].' '.$row[1].' '.$row[2].' con número de empleado '.$NumEmpleado.' adscrito a la División de '.$row[5].', solicito de manera voluntaria al Sindicato Único de Trabajadores Académicos del Tecnológico de Estudios Superiores de Ecatepec unirse como inversionista por la cantidad de $'.$row[3].' ( '.$letra.' 00/100 M.N.).'), 0, 'J');
$pdf->Ln(7);
$pdf->MultiCell(0, 7, utf8_decode('Comprometiendo el depósito en la cuenta del Sindicato S.U.T.A.T.E.S.E. al Banco XXXXXXXXXXXXXXXXXXXX a la cuenta XXXX-XXXX-XXXX-XXXX'), 0, 'J');
$pdf->Ln(10);
$pdf->Cell(190,10,'INTERESADO',0,1,'C');
$pdf->Ln(15);
$pdf->Cell(190,10,'___________________________',0,1,'C');
$pdf->Cell(190,5,'NOMBRE Y FIRMA',0,1,'C');

//$pdf->setAutoPageBreak(true,0); Salto de pagina cuando se termina la pagina
$pdf->Close();
$pdf->Output('D','ComprobantePagoInversionista.pdf');  
