        ...
        <?php
            if(isset($_POST['Insert'])){
                ...
                if(empty($employe) || empty($name) || empty($firstname) || empty($lastname) || empty($date) || empty($curp) || empty($rfc) || empty($dateIT) || empty($email) || empty($cellphone) || empty($phone) || empty($street) || empty($number) || empty($municipality) || empty($suburb) || empty($state) || empty($cp) || empty($division) || empty($experience) || empty($pexperience)){
                    ...
                    // echo $employe, $name, $firstname, $lastname, $date, $curp, $rfc, $dateIT, $email, $cellphone, $phone, $street, $number, $municipality, $suburb, $state, $cp, $division, $pexperience, $experience, $lic, $cedulaLic, $institucionLic, $mae, $cedulaMae, $institucionMae, $doc, $cedulaDoc, $institucionDoc;
                    ...
                    if($tipoN != "" && $tipoC != "" && $tipoP != ""){
                        ...
                        // }else if ($tipoP != 'image/jpg' || $tamanioP >= 204800){
                        //     echo '<script> Swal.fire({icon: "error", title: "Error...", text: "¡La foto no corresponde al archivo o es demasiado grande! Por favor, intente de nuevo", showConfirmButton: true, confirmButtonText: "Cerrar"}).
                        //         then(function(result){
                        //         if(result.value){                   
                        //         window.location = "registro.php";
                        //             }
                        ...
                        //         </script>';
                            ...
                            // echo "Se han insertado los siguientes valores: $employe, $name, $firstname, $lastname, $date, $curp, $rfc, $dateIT, $email, $cellphone, $phone, $street, $number, $municipality, $suburb, $state, $cp, $division, $experience, $pexperience, $lic, $cedulaLic, $institucionLic, $mae, $cedulaMae, $institucionMae, $doc, $cedulaDoc, $institucionDoc";
                ...
                // else{
                //     echo $tamanioN, $tamanioC;
                    // echo $employe, $nombre, $tipo, $tamanio, $ruta, $subida;
                //     $ejecucion = $conn->query("INSERT INTO empleado (NumEmpleado, Credencial) VALUES ('".$employe."', '".$subida."')");
                //         if ($ejecucion === TRUE) {
                //             echo 'el dato se inserto';
                //         }else{
                //             echo mysqli_error($conn);
                //         }
            ...
            // }
                ...
                // $cred = $_FILES['cred'];
                // echo "$employe, $name, $firstname, $lastname, $date, $curp, $rfc, $dateIT, $email, $cellphone, $phone, $street, $number, $municipality, $suburb, $state, $cp, $division, $experience, $pexperience ";
        ?>
        </div>
    </form>
